...
public class PneumaticRegistry {
    ...
    public static interface IPneumaticCraftInterface {
         ...
         * hacking, so the user can only hack an enderman once (more times wouldn't have any effect). This is mostly
         ...
         * returns true). With this method you can register custom blocks to allow the drone to pathfind through them.
         * If the block requires any special handling, like allow pathfinding on certain conditions, you can pass a
         * IPathFindHandler with the registry.
         * 
         * @param block
        ...
        /**
         * Will spawn in a Drone a distance away from the given coordinate. The drone is programmed to travel to go to 5
         * blocks above the specified y level, and drop the deliveredStacks. When there isn't a clear path for the items
         * to fall these 5 blocks the Drone will deliver at a y level above the specified y that _is_ clear. If no clear
         * blocks can be found (when there are only solid blocks), the Drone will drop the items very high up in the air
         * instead, and drop them there.
         *
         * When the Drone is tried to be catched by a player (by wrenching it), the drone will only the drop the items
         * that it was delivering (or none if it dropped those items already). The Drone itself never will be dropped.
         *
         * @param x
         * @param y
         * @param z
         * @param deliveredStacks stacks that are delivered by the drone. When no stacks, or more than 65 stacks are
         *                        given, this will generate a IllegalArgumentException.
         ...
         * @return The amount of Security Stations that disallow interaction for the given player. This method throws an
         *         IllegalArgumentException when tried to be called from the client side!
         */
        ...
        /**
         * Used to register a liquid that represents liquid XP (like MFR mob essence, OpenBlocks liquid XP). This is
         * used in the Aerial Interface to pump XP in/out of the player.
         * 
         * @param fluid
