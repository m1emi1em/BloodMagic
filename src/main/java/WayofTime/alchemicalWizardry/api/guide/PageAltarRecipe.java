...
public class PageAltarRecipe extends PageBase {
    ...
    public void draw(Book book, CategoryAbstract category, EntryAbstract entry, int guiLeft, int guiTop, int mouseX,
        ...
        guiBase.drawCenteredString(
                fontRenderer,
                StatCollector.translateToLocal("text.recipe.altar"),
        ...
        if (output.getItem() == Item.getItemFromBlock(Blocks.fire)) {
            ...
            guiBase.drawCenteredString(
                    fontRenderer,
                    StatCollector.translateToLocal("bm.string.tier") + ": " + String.valueOf(tier),
                    guiLeft + guiBase.xSize / 2,
                    guiTop + 4 * guiBase.ySize / 6 + 15,
                    0);
            guiBase.drawCenteredString(
                    fontRenderer,
                    "LP: " + String.valueOf(bloodRequired),
                    guiLeft + guiBase.xSize / 2,
                    guiTop + 4 * guiBase.ySize / 6 + 30,
                    0);
        }
        guiBase.drawCenteredString(
                fontRenderer,
                String.format(StatCollector.translateToLocal("text.recipe.altar.tier"), String.valueOf(tier)),
                guiLeft + guiBase.xSize / 2,
                guiTop + 4 * guiBase.ySize / 6,
                0);
        guiBase.drawCenteredString(
                fontRenderer,
                String.format(
                        StatCollector.translateToLocal("text.recipe.altar.bloodRequired"),
                        String.valueOf(bloodRequired)),
                guiLeft + guiBase.xSize / 2,
                guiTop + 4 * guiBase.ySize / 6 + 15,
                0);
    }
