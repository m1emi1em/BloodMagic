...
public class EntityAITargetAggroCloaking extends EntityAITargetAggro {
    ...
    public EntityAITargetAggroCloaking(
            EntityDemon par1EntityDemon, Class par2Class, int par3, boolean par4, int cloak) {
    ...
    public boolean shouldExecute() {
        ...
        if (target != null && target.isPotionActive(AlchemicalWizardry.customPotionBoost.id)) {
            cloakActive = target.getActivePotionEffect(AlchemicalWizardry.customPotionBoost)
                            .getAmplifier()
                    >= cloakLevel;
        }
