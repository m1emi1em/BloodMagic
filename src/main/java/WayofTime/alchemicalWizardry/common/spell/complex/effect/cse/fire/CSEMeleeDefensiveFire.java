...
public class CSEMeleeDefensiveFire extends ComplexSpellEffect {
    ...
    public void modifyParadigm(SpellParadigm parad) {
        if (parad instanceof SpellParadigmMelee) {
            ((SpellParadigmMelee) parad)
                    .addWorldEffect(new MeleeDefensiveFire(powerEnhancement, potencyEnhancement, costEnhancement));
        }
    }
    ...
    public int getCostOfEffect() {
        return (int)
                (30 * (1.5 * potencyEnhancement + 1) * (3 * powerEnhancement + 1) * Math.pow(0.85, costEnhancement));
    }
}
