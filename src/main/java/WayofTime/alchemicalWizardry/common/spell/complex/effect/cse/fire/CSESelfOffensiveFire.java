...
public class CSESelfOffensiveFire extends ComplexSpellEffect {
    ...
    public void modifyParadigm(SpellParadigm parad) {
        if (parad instanceof SpellParadigmSelf) {
            ((SpellParadigmSelf) parad)
                    .addSelfSpellEffect(new SelfOffensiveFire(powerEnhancement, potencyEnhancement, costEnhancement));
        }
    }
    ...
    public int getCostOfEffect() {
        return (int)
                (300 * (3 * powerEnhancement + 1) * (2 * potencyEnhancement + 1) * Math.pow(0.85, costEnhancement));
    }
}
