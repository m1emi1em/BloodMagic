...
public class ToolEnvironmentalWind extends OnBreakBlockEffect {
    ...
    @Override
    public int onBlockBroken(ItemStack container, World world, EntityPlayer player, Block block, int meta, int x, int y,
            int z, ForgeDirection sideBroken) {
        double vertRange = 0.5 + (this.powerUpgrades * this.powerUpgrades + this.powerUpgrades) / 2;

        List<EntityItem> itemList = SpellHelper
                .getItemsInRange(world, x + 0.5f, y + 0.5f, z + 0.5f, vertRange, vertRange);
