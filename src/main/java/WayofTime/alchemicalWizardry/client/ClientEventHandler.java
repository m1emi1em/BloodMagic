...
public class ClientEventHandler {
    ...
    //    {
    //    	if (this.mc.thePlayer.worldObj.isRemote && this.mc.gameSettings.thirdPersonView == 0 &&
    // !this.mc.renderViewEntity.isPlayerSleeping() && !this.mc.gameSettings.hideGUI &&
    ...
    //  {
    //  	ModelBiped model = ((BoundArmour)ModItems.boundPlate).getArmorModel(event.entityPlayer, new
    // ItemStack(ModItems.boundPlate), 2);
    //  	String texture = ((BoundArmour)ModItems.boundPlate).getArmorTexture(new ItemStack(ModItems.boundPlate),
    // event.entityPlayer, 2, "");
    //
    ...
    public void onKeyInput(InputEvent.KeyInputEvent event) {
        ...
        //            ClientToServerPacketHandler.INSTANCE.sendToServer(new
        // MessageKeyPressed(MessageKeyPressed.Key.OMEGA_ACTIVE));
        //        }
    }
    ...
    public void onOverlayEvent(RenderBlockOverlayEvent event) {
        if (event.overlayType == RenderBlockOverlayEvent.OverlayType.WATER
                && event.player.isPotionActive(
                        AlchemicalWizardry.customPotionAmphibian.id)) // TODO Placeholder for new potion effect
